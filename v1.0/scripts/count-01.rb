...
def main
  ...
  file_count = 0
  line_count = 0
  Dir.foreach('../tmp/monthly') { |file_name|
    next if file_name.match(/^\./)
    file_count += 1
    File.open('../tmp/monthly/' + file_name, 'r').each { |line|
      line_count += 1
    ...
    #user_ids.uniq!
    count = file_count
    puts "Processing %s, %d files have been processed" % [file_name, count] if count % 100 == 0
    puts "%d users have been recorded" % user_ids.size if count % 500 == 0
    puts "%d lines" % line_count if count % 500 == 0
  }

  puts "%d users" % user_ids.size
  #File.open('../tmp/usercount.txt', 'w'){ |file|
  #  user_ids.each { |uid,_|
  #    file.puts uid
  #  }
  #}

  puts "%d lines" % line_count
